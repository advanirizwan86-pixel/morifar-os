...
import {ApprovalAssistant} from "@/components/intelligence-panels";
...
import {getApprovalObservation} from "@/server/services/intelligence";
...
export default async function ApprovalsPage({searchParams}:{searchParams:Promise<{status?:string}>}){
 ...
 return <div className="page">
  ...
  <section className="ops-card-list approvals-list">{approvals.map(item=><article className="ops-application-card" key={item.id}><header><div><IconShieldCheck size={18}/><h2>{item.title}</h2><p>{item.entity_type} - {item.entity_id}</p></div><span className={`priority ${item.priority}`}>{item.priority}</span></header><dl><div><dt>STATUS</dt><dd>{item.status.replaceAll("_"," ")}</dd></div><div><dt>REQUESTED BY</dt><dd>{item.requested_by_name??"System"}</dd></div><div><dt>ASSIGNED</dt><dd>{item.assigned_to_name??"Unassigned"}</dd></div><div><dt>AI</dt><dd>{item.ai_name??"None"}</dd></div></dl><p className="ops-copy">{item.reason??"No reason recorded."}</p><ApprovalAssistant observations={getApprovalObservation(item.id)}/><form action={decideApproval} className="ops-inline-form"><input type="hidden" name="id" value={item.id}/><select name="decision" defaultValue={item.status} aria-label={`Decision for ${item.title}`}><option value="approved">Approve</option><option value="rejected">Reject</option><option value="changes_requested">Request changes</option><option value="escalated">Escalate</option></select><input name="notes" placeholder="Decision notes" aria-label={`Decision notes for ${item.title}`}/><button aria-label={`Record decision for ${item.title}`}><IconChecks size={14}/>Record</button></form></article>)}</section>
